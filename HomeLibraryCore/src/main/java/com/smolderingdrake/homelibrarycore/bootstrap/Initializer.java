...
public class Initializer implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private static final Book ADAPTIVE_CODE = Book.builder().isbn("9788328338708").description("Adaptive Code.\nNothing more").count(1).genre(Genre.SCIENCE).authors(List.of(GREG_MCLEAN_HALL)).title("Adaptive Code").build();
    private static final Book SPRING_IN_ACTION_4TH = Book.builder().isbn("9788328308497").description("Spring in Action.\n4th edition.").count(1).genre(Genre.SCIENCE).authors(List.of(CRAIG_WALLS)).title("Spring in Action. 4th edition").build();
    private static final Book SPRING_IN_ACTION_5TH = Book.builder().isbn("9788328356061").description("Spring in Action.\n5th edition.").count(1).genre(Genre.SCIENCE).authors(List.of(CRAIG_WALLS)).title("Spring in Action. 5th edition").build();
